...
public class SentryImporter {
    ...
    public static int PerformImport() {
        ...
        for ( NPC npc : CitizensAPI.getNPCRegistry() ) {
            if ( !npc.hasTrait( net.aufdemrand.sentry.SentryTrait.class ) ) continue;
            ...
            for ( String t : sentry.validTargets ) {
                if ( t.contains( "ENTITY:ALL" ) )
                    ...
                    if ( sections[0].equals( "NPC" ) ) {
                        ...
                        CommandHandler.getCommand( "towny" ).call( null, npc.getName(), newSentry, 0, "towny", "join", sections[1] );
                    }
...
// Sentries doesn't support ignoring events, just don't add them as targets.
//                    else if ( sections[0].equals( "EVENT" ) ) { }
                    
                    else if ( sections[0].equals( "GROUP" ) ) {
                        CommandHandler.getCommand( "group" ).call( null, null, newSentry, 0, "group", "ignore", sections[1] ); 
                    }
                    else if ( sections[0].equals( "FACTION" ) ) {
                        CommandHandler.getCommand( "faction" ).call( null, null, newSentry, 0, "faction", "ignore", sections[1] );
                    }
                    // Ignoring towny members is not currently implemented.
//                    else if ( sections[0].equals( "TOWN" ) ) {}
                    // This target doesn't work well with how Towny works.
//                    else if ( sections[0].equals( "NATION" ) ) { }
                    else if ( sections[0].equals( "WARTEAM" ) ) {
                        CommandHandler.getCommand( "war" ).call( null, null, newSentry, 0, "war", "ignore", sections[1] );
                    }
                    else if ( sections[0].equals( "TEAM" ) ) {
                        CommandHandler.getCommand( "scoreboard" ).call( null, null, newSentry, 0, "scoreboard", "ignore", sections[1] );
                    }
                    else if ( sections[0].equals( "CLAN" ) ) {
                        CommandHandler.getCommand( "clan" ).call( null, null, newSentry, 0, "clan", "ignore", sections[1] );
                    }
                }
            }
        }
