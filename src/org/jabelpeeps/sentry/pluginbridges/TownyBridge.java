...
public class TownyBridge extends PluginBridge {
    ...
    public static String[] getResidentTownyInfo( Player player ) {
        ...
        try {
            Resident resident = TownyUniverse.getDataSource()
    ...
    public static boolean isNationEnemy( String Nation1, String Nation2 ) {
        ...
        if ( !Nation1.equalsIgnoreCase( Nation2 )
            ...
            try {
                Nation theNation1 = TownyUniverse.getDataSource()
                        .getNation( Nation1 );
                Nation theNation2 = TownyUniverse.getDataSource()
                        .getNation( Nation2 );

                if ( theNation1.hasEnemy( theNation2 )
                        || theNation2.hasEnemy( theNation1 ) )
                    return true;
    ...
    // {
    // String natname = TownyBridge.getNationNameForLocation(
    // inst.myNPC.getEntity().getLocation() );
    // if ( natname != null ) {
    // arg += ":" + natname;
    // }
    // else {
    // player.sendMessage( ChatColor.RED + "Could not get Nation for this NPC's
    // location" );
    // return true;
    // }
    // }
